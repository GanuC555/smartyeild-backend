...
```
AppModule
├── ConfigModule (global, dotenv)
├── MongooseModule (global, connection)
├── BullModule (global, Redis connection)
├── ThrottlerModule (rate limiting)
│
├── AuthModule
│   ├── AuthController    POST /auth/nonce, POST /auth/verify, POST /auth/refresh
│   ├── AuthService       SIWE verify, JWT issue, nonce management
│   ├── JwtStrategy       (Passport)
│   └── WsJwtGuard        (WebSocket auth)
│
├── VaultModule
│   ├── VaultController   GET /vaults, GET /vaults/:id, POST /vaults/:id/deposit-quote
│   ├── VaultService      Deposit/withdraw orchestration, share calculations
│   └── VaultGateway      WebSocket: vault state updates
│
├── PortfolioModule
│   ├── PortfolioController  GET /portfolio, GET /portfolio/history
│   └── PortfolioService     User position aggregation across chains
│
├── StrategyModule
│   ├── StrategyController  GET /strategies, GET /strategies/:id/metrics
│   └── StrategyService     Strategy state, APY tracking
│
├── AIAgentModule
│   ├── AgentService        Claude tool-use loop (see above)
│   ├── AgentProcessor      BullMQ worker: allocation-decision, reconcile
│   ├── MarketDataService   Fetch OneDex/OnePredict metrics
│   └── ExecutorService     Sign and submit rebalance txns
│
├── CardModule
│   ├── CardController    POST /cards, GET /cards, GET /cards/:id
│   ├── CardService       Card lifecycle management
│   ├── CardProcessor     BullMQ: settlement reconciliation
│   ├── LithicWebhook     POST /webhooks/lithic (real-time auth + events)
│   └── CardProviderFactory  Returns ICardProvider (Lithic or Marqeta)
│
├── TransactionModule
│   ├── TransactionController  GET /transactions
│   └── TransactionService     Transaction history aggregation
│
├── BlockchainModule
│   ├── ChainRegistry
│   ├── OneChainAdapter        ← Phase 1 (hackathon)
│   ├── EVMAdapter             ← Phase 4 (post-hackathon, skeleton stubbed now)
│   └── BlockchainService  Higher-level ops using registry
│
└── NotificationModule
    └── NotificationGateway  WebSocket hub (Socket.io)
```
...
### **E2E Testing**
- use the bash script to test each feature , each end point end to end after the complete feature .
- never use any mock data for teh testing always test with the real compoenents and data .
